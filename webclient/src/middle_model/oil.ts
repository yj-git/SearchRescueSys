...
class OilMidModel {
    ...
    public get windVal(): number {
        this._windVal=Number(Math.sqrt(Math.pow(this.wind.x, 2) + Math.pow(this.wind.y, 2)).toFixed(2));
    ...
    public get currentVal():number{
        this._currentVal=Number((Math.sqrt(Math.pow(this.current.x, 2) + Math.pow(this.current.y, 2))).toFixed(2));
