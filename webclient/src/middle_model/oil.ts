...
class OilMidModel {
    ...
    public get windDir(): number {
        const arctan = Math.atan2(this.wind.y, this.wind.x)
        const dir = Number((arctan * (180 / Math.PI)).toFixed(2))
        this._windDir = dir
    ...
    public get currentVal(): number {
        this._currentVal = Number(
            Math.sqrt(Math.pow(this.current.x, 2) + Math.pow(this.current.y, 2)).toFixed(2)
        )
    ...
    public get currentDir(): number {
        const arctan = Math.atan2(this.current.y, this.current.x)
        const dir = Number((arctan * (180 / Math.PI)).toFixed(2))
